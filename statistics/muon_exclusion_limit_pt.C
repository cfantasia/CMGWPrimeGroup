...
.L muon_exclusion_limit.C++
 ...
 Modified by Darren Puigh; 22 June, 2010

 Modified by Christos Leonidopoulos for W'->mu limit: November 2010
...
static Double_t MinLike = 1.e-6, MyPrecision = 1.e-5;
static Int_t N;
static bool lGauss = kFALSE, plot = kTRUE, plotL=!true;
static Int_t I = 0;
...
void muon_exclusion_limit_pt_jet50( int backgroundModel_ = 2, double BackgroundError_ = -1, bool makePlot = true ){
  ...
  sig_eff_err[0] = 0.267937;
  sig_eff_err[1] = 0.423651;
  sig_eff_err[2] = 0.466326;
  sig_eff_err[3] = 0.477063;
  sig_eff_err[4] = 0.478452;
  sig_eff_err[5] = 0.477221;
  sig_eff_err[6] = 0.475257;
  sig_eff_err[7] = 0.473348;
  sig_eff_err[8] = 0.470022;
  sig_eff_err[9] = 0.47107;
  sig_eff_err[10] = 0.475109;
  ...
  double tevatron_limit = 1.1;

  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////


  std::vector<std::string> bkgmodel;
  bkgmodel.push_back("Gaus");
  bkgmodel.push_back("LogNormal");
  bkgmodel.push_back("Gamma");

  int mybkg = ( (backgroundModel_<0) || (backgroundModel_>2) ) ? 2 : backgroundModel_;
  ...
  char *gifname = new char[100];
  std::string plotname;


  double expXsecLimit[Nsignal];
  double obsXsecLimit[Nsignal];

  for( int j=0; j<Nsignal; j++ ){
    expXsecLimit[j]=0; obsXsecLimit[j]=0;
  }
  ...
  for( int j=0; j<Nsignal; j++ ){
    ...
    std::cout << "     MW = " << mass[j] << std::endl;

    expXsecLimit[j] =  CLA(intLumi,0.11*intLumi,sig_eff[j],sig_eff_err[j],bkg,bkg_err,mybkg);//bckmodel = 0
    obsXsecLimit[j] = CL95(intLumi,0.11*intLumi,sig_eff[j],sig_eff_err[j],bkg,bkg_err,data,usegaus,mybkg);//gauss = false, bckmodel = 0
  }
  ...
  TH1D* h_exp_xsec_limit = new TH1D("h_exp_xsec_limit",";Mass (TeV/c^{2});#sigma #upoint BR(W' #rightarrow #mu#nu) (pb)", Nbins_limit, xmin, xmax );
  TH1D* h_obs_xsec_limit = new TH1D("h_obs_xsec_limit",";Mass (TeV/c^{2});#sigma #upoint BR(W' #rightarrow #mu#nu) (pb)", Nbins_limit, xmin, xmax );
  TH1D* h_theory_xsec = new TH1D("h_theory_xsec",";Mass (TeV/c^{2});#sigma #upoint BR(W' #rightarrow #mu#nu) (pb)", Nbins_limit, xmin, xmax );
  TH1D* h_theory_xsec_band = new TH1D("h_theory_xsec_band",";Mass (TeV/c^{2});#sigma #upoint BR(W' #rightarrow #mu#nu) (pb)", Nbins_limit, xmin, xmax );

  for( int j=0; j<Nsignal; j++ ){
    h_exp_xsec_limit->Fill(mass[j],expXsecLimit[j]);
    h_obs_xsec_limit->Fill(mass[j],obsXsecLimit[j]);
    h_theory_xsec->Fill(mass[j],xsec[j]);
    int mbin = h_theory_xsec_band->FindBin(mass[j]+0.0001);
    h_theory_xsec_band->SetBinError(mbin,0.1*xsec[j]);
    h_theory_xsec_band->SetBinContent(mbin,xsec[j]);
  }

  if( makePlot ){

    h_exp_xsec_limit->SetMarkerStyle(1);h_exp_xsec_limit->SetMarkerColor(kBlue);
    h_obs_xsec_limit->SetMarkerStyle(1);h_obs_xsec_limit->SetMarkerColor(kRed);
    h_theory_xsec_band->SetMarkerStyle(1); 
    h_theory_xsec->SetMarkerStyle(1); 
    h_theory_xsec->SetMarkerColor(1);

    double ymin = 0.005;
    double ymax = 20.;
    h_exp_xsec_limit->GetYaxis()->SetRangeUser(ymin,ymax);

    h_exp_xsec_limit->SetStats(0);
    h_theory_xsec->SetStats(0);
    h_exp_xsec_limit->SetLineColor(kBlue);
    h_exp_xsec_limit->SetLineWidth(2);
    h_exp_xsec_limit->SetLineStyle(2);
    h_obs_xsec_limit->SetLineWidth(2);
    h_obs_xsec_limit->SetLineColor(kRed);
    h_theory_xsec->SetLineColor(1);
    h_theory_xsec->SetLineWidth(2);
    h_theory_xsec->SetFillColor(8);
    h_theory_xsec_band->SetFillColor(8);


    TBox* tev_excl = new TBox();
    tev_excl->SetFillColor(kGray);
 
    gPad->RedrawAxis();

    h_exp_xsec_limit->Draw("p");
    tev_excl->DrawBox(xmin,ymin,tevatron_limit,ymax);
    h_theory_xsec_band->Draw("e5same");
    h_theory_xsec->Draw("psame");
    h_exp_xsec_limit->Draw("psameaxis");
    h_obs_xsec_limit->Draw("psame");
    c1->Modified();
    ...
    TLine* line_the = new TLine();

    line_exp->SetLineColor(kBlue);
    line_exp->SetLineWidth(2);
    line_exp->SetLineStyle(2);
    line_obs->SetLineColor(kRed);
    line_obs->SetLineWidth(2);
    line_the->SetLineColor(1);
    line_the->SetLineWidth(2);
    line_the->SetLineStyle(1);

    for( int j=0; j<Nsignal-1; j++ ){
      line_exp->DrawLine( mass[j], expXsecLimit[j], mass[j+1], expXsecLimit[j+1] );
      line_obs->DrawLine( mass[j], obsXsecLimit[j], mass[j+1], obsXsecLimit[j+1] );
      line_the->DrawLine( mass[j], xsec[j], mass[j+1], xsec[j+1] );
    }
    ...
    std::string cmsinfo = "CMS Preliminary 2010";
    TLatex CMSInfoLatex(0.30, 0.33, cmsinfo.c_str());
    CMSInfoLatex.SetNDC();
    CMSInfoLatex.SetTextFont(42);
    CMSInfoLatex.SetTextSize(0.04);

    std::string lumiinfo = "#int L dt = 34.2 pb^{-1}";
    TLatex LUMIInfoLatex(0.37, 0.27, lumiinfo.c_str());
    LUMIInfoLatex.SetNDC();
    LUMIInfoLatex.SetTextFont(42);
    LUMIInfoLatex.SetTextSize(0.03);

    std::string comeinfo = "#sqrt{s} = 7 TeV";
    TLatex COMEInfoLatex(0.40, 0.20, comeinfo.c_str());
    COMEInfoLatex.SetNDC();
    COMEInfoLatex.SetTextFont(42);
    COMEInfoLatex.SetTextSize(0.03);


    TLegend *legend = new TLegend(0.52,0.67,0.92,0.92);
    legend->AddEntry(tev_excl,"Tevatron Exclusion (5.3 fb^{-1})","f");
    legend->AddEntry(h_exp_xsec_limit,"Expected Limit at 95% CL","l");
    legend->AddEntry(h_obs_xsec_limit,"Observed Limit at 95% CL","l");
    legend->AddEntry(h_theory_xsec,   "Theoretical Cross Section","fl");
    ...
    if( BackgroundError_>0.00001 ){
      int bkgError = int( BackgroundError_ * 100 + 0.0001 );
      plotname = "h_muon_xsec_limit_Err";
      sprintf(gifname,"%s%d_%s_intLumi%dpb.gif",plotname.c_str(),bkgError,bkgmodel[mybkg].c_str(),int(intLumi));
      c1->Print(gifname);
      sprintf(gifname,"%s%d_%s_intLumi%dpb.eps",plotname.c_str(),bkgError,bkgmodel[mybkg].c_str(),int(intLumi));
      c1->Print(gifname);
    }
    else{
      plotname = "h_muon_xsec_limit_Err_" + bkgmodel[mybkg];
      sprintf(gifname,"%s_intLumi%dpb.gif",plotname.c_str(),int(intLumi));
      c1->Print(gifname);
      sprintf(gifname,"%s_intLumi%dpb.eps",plotname.c_str(),int(intLumi));
      c1->Print(gifname);
    }
  ...
  int left_obs = -1;
  for( int j=0; j<Nsignal-1; j++ ){
    if( obsXsecLimit[j]<xsec[j] && obsXsecLimit[j+1]>xsec[j+1] ) left_obs = j;
    if( expXsecLimit[j]<xsec[j] && expXsecLimit[j+1]>xsec[j+1] ) left_exp = j;
  }


  double exp_mass = 0.0;
  if( left_exp>0 ){
    double x1 = mass[left_exp];
    double x2 = mass[left_exp+1];
    double y1_exp = expXsecLimit[left_exp];
    double y2_exp = expXsecLimit[left_exp+1];
    double y1_the = xsec[left_exp];
    double y2_the = xsec[left_exp+1];

    double A_exp = (y2_exp-y1_exp) * 1./(x2-x1);
    double B_exp = y1_exp - A_exp*x1;
    double A_the = (y2_the-y1_the) * 1./(x2-x1);
    double B_the = y1_the - A_the*x1;

    exp_mass = (B_exp - B_the) * 1./(A_the - A_exp);

    std::cout << "   left_exp = " << left_exp << ", exp_mass = " << exp_mass << std::endl;
  }

  double obs_mass = 0.0;
  if( left_obs>0 ){
    double x1 = mass[left_obs];
    double x2 = mass[left_obs+1];
    double y1_obs = obsXsecLimit[left_obs];
    double y2_obs = obsXsecLimit[left_obs+1];
    double y1_the = xsec[left_obs];
    double y2_the = xsec[left_obs+1];

    double A_obs = (y2_obs-y1_obs) * 1./(x2-x1);
    double B_obs = y1_obs - A_obs*x1;
    double A_the = (y2_the-y1_the) * 1./(x2-x1);
    double B_the = y1_the - A_the*x1;

    obs_mass = (B_obs - B_the) * 1./(A_the - A_obs);

    std::cout << "   left_obs = " << left_obs << ", obs_mass = " << obs_mass << std::endl;
  }


  std::cout << " *********************************************************** " << std::endl;
  std::cout << "        intLumi = " << intLumi << " /pb " << std::endl;

  std::cout << "           data = " << data << std::endl; 
  std::cout << "     background = " << bkg << " +/- " << bkg_err << std::endl; 

  for( int j=0; j<Nsignal; j++ ){
    std::cout << " --------------------------------------------------------- " << std::endl;
    std::cout << "    Results for W' with mass = " << mass[j] << " TeV/c^2" << std::endl;
    std::cout << "       Cross section  = " << xsec[j] << " pb " << std::endl;
    std::cout << "       Expected Limit = " << expXsecLimit[j] << " pb " << std::endl;
    std::cout << "       Observed Limit = " << obsXsecLimit[j] << " pb " << std::endl;
  }

  std::cout << " *********************************************************** " << std::endl;
  std::cout << "  Expected Limit = " << exp_mass << " TeV " << std::endl; 
  std::cout << "  Observed Limit = " << obs_mass << " TeV " << std::endl; 
  std::cout << " *********************************************************** " << std::endl;
...
//
	...
	B0 = bck;
	sB = sbck;
	epsilon = TMath::Max(MyPrecision/ilum,1.e-4);
	//
	lGauss = gauss;
