		      ...
		      )
{
  ...
  if(detector_conditions == 1)
    ...
    {
      // NB: since the four samples are not independent processes, we artificially reduce
      // the W cross section (11.9 nb) by 4 (2.975 nb) so that the sum gets scaled properly
      w_files.push_back(wprime::InputFile(2975, 2604000));
      w_files.push_back(wprime::InputFile(2975, 2280000));
      w_files.push_back(wprime::InputFile(2975, 1590000));
      w_files.push_back(wprime::InputFile(2975, 2028000));
    }
