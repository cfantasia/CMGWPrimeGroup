  ...
  isolator_(iConfig.exists("userIsolation") ? iConfig.getParameter<edm::ParameterSet>("userIsolation") : edm::ParameterSet(), false)
{
  ...
  if(addTeVRefits_){
    ...
    defaultTeVSrc_ = iConfig.getParameter<edm::InputTag>("defaultTeVSrc");
    dytSrc_ = iConfig.getParameter<edm::InputTag>("dytSrc");
  }
  ...
  embedDefaultTeVMuon_ = iConfig.getParameter<bool>( "embedDefaultTeVMuon" );
  embedDytMuon_ = iConfig.getParameter<bool>( "embedDytMuon" );
  embedCocktailMuon_ = iConfig.getParameter<bool>( "embedCocktailMuon" );
  ...
  if( useParticleFlow_ ){
	...
	if (!pickyMap.failedToGet()) {
	  ...
	  if (it_picky != pickyMap->end()) 
	    aMuon.setPickyMuon(it_picky->val);
	  else TeVfailed = true;	  
	...
	if (!tpfmsMap.failedToGet()) {
	  ...
	  if (it_tpfms != tpfmsMap->end()) 
	    aMuon.setTpfmsMuon(it_tpfms->val);
	  else TeVfailed = true;
	...
	if (!defaultTeVMap.failedToGet()) {
	  ...
	  if (it_defTeV != defaultTeVMap->end())
	    aMuon.setDefaultTeVMuon(it_defTeV->val);
	  else TeVfailed = true;
