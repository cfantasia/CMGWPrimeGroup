...
Muon::Muon() :
    ...
    embeddedDefaultTeVMuon_(false),
    embeddedDytMuon_(false),
    embeddedCocktailMuon_(false),
    pickyMuonRef_(),
    tpfmsMuonRef_(),
    defaultTeVMuonRef_(),
    dytMuonRef_(),
...
reco::TrackRef Muon::defaultTeVMuon() const {
  if (embeddedDefaultTeVMuon_) {
    return reco::TrackRef(&defaultTeVMuon_, 0);
  } else {
    return defaultTeVMuonRef_;
  }
}

/// reference to DYT refit
reco::TrackRef Muon::dytMuon() const {
  if (embeddedDytMuon_) {
...
/// reference to cocktail
reco::TrackRef Muon::cocktailMuon() const {
  if (embeddedCocktailMuon_) {
    return reco::TrackRef(&cocktailMuon_, 0);
  } else {
    return cocktailMuonRef_;
  }
}
...
void Muon::embedDefaultTeVMuon() {
  ...
  if (defaultTeVMuonRef_.isNonnull()) {
      defaultTeVMuon_.push_back(*defaultTeVMuonRef_);
      embeddedDefaultTeVMuon_ = true;
  }
}
...
void Muon::embedDytMuon() {
  dytMuon_.clear();
  if (dytMuonRef_.isNonnull()) {
      dytMuon_.push_back(*dytMuonRef_);
      embeddedDytMuon_ = true;
  }
}

/// embed the cocktail Track
void Muon::embedCocktailMuon() {
  cocktailMuon_.clear();
  if (cocktailMuonRef_.isNonnull()) {
      cocktailMuon_.push_back(*cocktailMuonRef_);
